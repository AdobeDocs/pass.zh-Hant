...
>[!NOTE]
>
> 請注意，此功能用於沒有完整預檢授權支援的MVPD時，需要事先與Adobe和MVPD商定。 在這些MVPD上使用預檢授權將屬於[這裡](/help/authentication/mvpd-preflight-authz.md#intro)所述的「最壞情況」，並可能導致效能問題和緩慢的回應時間。</br>
...
快取機制會略過網路呼叫，改善後續呼叫的效能時間。 此外，快取清單可在驗證程式中預先填入。  （如需設定此情境的詳細資訊，請參閱MVPD整合指南授權一節中的[Preflight授權整合](/help/authentication/authz-usecase.md#preflight_authz_int)）。
...
Preflight API的效能取決於特定的MVPD實作。  如需實作選項，請參閱MVPD Integration Guide的Authorization一節中的[Preflight Authorization Integration](/help/authentication/authz-usecase.md#preflight_authz_int)。
