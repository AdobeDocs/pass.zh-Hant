...
Android AccessEnabler是Java Android資料庫，可讓行動應用程式使用Adobe Pass驗證來進行TV Everywhere的權益服務。 Android實施包含定義軟體權利檔案API的AccessEnabler介面，以及描述程式庫觸發之回呼的EntitlementDelegate通訊協定。 介面與通訊協定參照於一個通用名稱下：AccessEnabler Android程式庫。

## Android需求 {#reqs}

如需與Android平台和Adobe Pass驗證相關的目前技術需求，請參閱 [平台/裝置/工具需求](#android)，或參閱Android SDK下載專案隨附的發行說明。
