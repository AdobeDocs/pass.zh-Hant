...
iOS AccessEnabler是Objective C iOS/tvOS資料庫，可讓行動應用程式使用Adobe Pass驗證來進行TV Everywhere的權益服務。 實施包含 *AccessEnabler* 定義權益API的介面，以及 *EntitlementDelegate* 和 *[EntitlementStatus](#ios%20entitlement%20status)* 說明程式庫觸發之回呼的通訊協定。 介面與通訊協定在同一個一般名稱下稱為：AccessEnabler程式庫。

## iOS和tvOS需求 {#reqs}

如需與iOS和tvOS平台以及Adobe Pass驗證相關的目前技術需求，請參閱 [平台/裝置/工具需求](#ios)，並參閱SDK下載隨附的發行說明。 在本頁面的其餘部分中，您會看到註解適用於特定SDK版本及更新版本之變更的部分。 例如，以下是有關1.7.5 SDK的合法附註：
...
1. 您的應用程式會呼叫AccessEnabler的 `logout() `API方法。 登出是一系列HTTP重新導向作業的結果，因為使用者需要同時從Adobe Pass驗證伺服器和MVPD伺服器登出。 由於此流程無法使用AccessEnabler程式庫發出的簡單HTTP要求來完成， `UIWebView/WKWebView or SFSafariViewController` 控制器必須具現化，才能遵循HTTP重新導向作業。
...
Token必須持續存在於相同應用程式的連續執行中。 這表示取得驗證和授權權杖且使用者關閉應用程式後，當使用者重新開啟應用程式時，應用程式可使用相同的權杖。 此外，這些代號最好在多個應用程式中持續儲存。 換言之，當使用者使用一個應用程式與特定的身分提供者登入（成功取得驗證和授權權杖）後，相同的權杖可以透過不同的應用程式使用，而且當使用者透過相同的身分提供者登入時，不再提示使用者輸入認證。 正是這種流暢的驗證/授權工作流程，使得Adobe Pass驗證解決方案成為真正的電視無處不在的實作。
...
請注意，此處包含AuthN和AuthZ權杖的格式僅供背景資訊使用。 Adobe Pass驗證可隨時變更這些權杖的結構。 程式設計師不需要知道AuthN和AuthZ權杖的確切結構即可實作其應用程式，因為AuthN和AuthZ權杖不會在本機裝置上公開。 短媒體權杖 *是* 公開至程式設計師的應用程式。
...
在上述XML清單中，請注意標題為 `simpleTokenFingerprint`. 此標籤的用途是儲存原生裝置ID個人化資訊。 AccessEnabler程式庫可取得這類個人化資訊，並在軟體權利檔案呼叫期間提供給Adobe Pass驗證服務。 此服務將使用此資訊並將其嵌入實際權杖，因此可以將權杖有效地繫結到特定裝置。 這的最終目標是要讓權杖不可跨裝置轉讓。
