...
* **使用者識別碼資訊，例如電子郵件地址傳播** （在驗證和授權流程中傳送使用者的電子郵件地址）。 Adobe Pass驗證需要電子郵件來繫結驗證和授權權杖(類似於 `device_ID`，所有呼叫均需要)。
...
>Adobe Pass驗證沒有內建機制可在X分鐘後停止免費串流。 Adobe Pass驗證將停止發行 **authorization** 和 **短媒體** Token使用者消耗Y可用資源後。 一旦「促銷臨時通過」過期，便由程式設計師來限制存取。
...
某些商業規則需要定期永久刪除「促銷臨時通行」。 為此，Adobe Pass驗證為程式設計師提供 *公共* 網頁API，如下所述：
