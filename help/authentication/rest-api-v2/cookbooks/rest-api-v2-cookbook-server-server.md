...
在伺服器對伺服器實作中，串流應用程式和程式設計師服務需要建立通訊協定，程式設計師服務才能：
* 唯一識別裝置上的串流應用程式
...
（[擷取可用的MVPD清單](../apis/configuration-apis/rest-api-v2-configuration-apis-retrieve-configuration-for-specific-service-provider.md)）
   * 程式設計人員服務可能會對MVPD清單實作篩選，並只顯示想要用來隱藏其他MVPD （TempPass、測試MVPD、開發中的MVPD等）
   * 程式設計師服務應傳回串流應用程式的篩選MVPD清單，以顯示選擇器，使用者選取MVPD
...
* 每個播放開始都需要執行步驟
* 串流應用程式會將此資訊傳遞至程式設計師服務
* 串流應用程式上的程式設計人員服務，請呼叫<b>/api/v2/{serviceProvider}/decision/authorize/{mvpd}</b><br>
...
* 如果串流應用程式沒有瀏覽器，或是與驗證時不同的執行個體，則流量可以停止，因為MVPD工作階段不會持續存在於瀏覽器快取中。
