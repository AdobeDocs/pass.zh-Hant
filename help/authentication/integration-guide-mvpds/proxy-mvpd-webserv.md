...
- `requestorIds` （選擇性） - requestorIds值將由Adobe提供。 需求是，代理的MVPD應至少與一個requestorId整合。 如果「requestorIds」標籤不存在於代理的MVPD元素上，則該代理的MVPD將與整合在Proxy MVPD下的所有可用請求者整合。
