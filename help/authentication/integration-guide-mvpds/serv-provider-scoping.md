...
由於Adobe Pass驗證扮演了程式設計師的SP角色，因此必須實作自訂，以啟用驗證請求的SP範圍設定。  此工作必須完成，以便MVPD可識別傳遞給MVPD身分提供者(IdP)的SAML宣告中的網路品牌。  範圍設定可透過下一節中說明的兩種方式之一來實施。
...
* **自訂範圍屬性方法。**  在此方法中，「請求者ID」會明確包含在SAML驗證請求中，作為自訂「範圍」屬性。
