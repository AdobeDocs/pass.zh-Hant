...
- *選項* — 包含應用程式ID值、訪客ID值無重新整理設定（背景登入登出）和MVPD設定(iFrame)的JSON物件。 所有值均為選用。
   ...
   2. 如果指定應用程式的唯一識別碼 — `applicationId` — 則會將該值新增至應用程式進行的所有後續呼叫，作為X-Device-Info HTTP標頭的一部分。 稍後可以使用適當的查詢從[ESM](/help/authentication/entitlement-service-monitoring-overview.md)報表擷取此值。
...
>[!NOTE]
> 
>如果您使用目前的[進階錯誤報告](/help/authentication/error-reporting.md)系統，則可以忽略傳送至此函式的errorCode引數。  不過，isAuthenticated旗標仍可用於追蹤軟體權利檔案流程中使用者的驗證狀態
