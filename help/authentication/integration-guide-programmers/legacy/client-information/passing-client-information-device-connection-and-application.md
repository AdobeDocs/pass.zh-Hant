...
| #mandatory | osName | #是 | 裝置的作業系統(OS)名稱。 | #值受到限制： Android Chrome OS Linux Mac OS X OpenBSD Roku OS Windows iOS tvOS webOS |
|            | osFamily | 是 | 裝置的作業系統(OS)群組名稱。 | #限制值： Android BSD Linux PlayStation OS Roku OS Symbian Tizen Windows iOS macOS tvOS webOS |
...
|            | browserVersion | 否 | 裝置的瀏覽器版本。 | e.g. 60.0.3112 |
...
|            | displayPpi | 否 | 裝置的實體熒幕畫素密度。 | e.g. 294 |
|            | diagonalScreenSize | 否 | 裝置的實體熒幕對角線尺寸（英吋）。 | e.g. 5.5, 10.1 |
|            | connectionIp | 否 | 用於傳送HTTP要求的裝置IP。 | e.g. 8.8.4.4 |
|            | connectionPort | 否 | 用於傳送HTTP要求的裝置連線埠。 | e.g. 53124 |
...
|   | displaywidth | DisplayInformation.ScreenWidthInRawPixel | 1920 |
|   | displayheight | DisplayInformation.ScreenHeightInRawPixel | 1080 |
