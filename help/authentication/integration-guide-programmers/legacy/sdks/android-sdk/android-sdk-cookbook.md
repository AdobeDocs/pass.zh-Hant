...
1. 呼叫[`logout()`](#$logout)將使用者登出。\
   ...
   答：  依照與驗證工作流程相同的模式，AccessEnabler網域會向UI應用程式層提出要求（透過`navigateToUrl()`回呼）來建立WebView控制項，並指示該控制項在後端伺服器上載入登出端點的URL。

   b.  UI必須再次監視WebView控制項的活動，並偵測控制項在經過數次重新導向時，載入應用程式的自訂URL （亦即： `http://adobepass.android.app/`）的時刻。 發生此事件後，UI應用程式層會關閉WebView並完成登出程式。
