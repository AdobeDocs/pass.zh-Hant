...
在大多數情況下，Adobe Pass驗證API會在回應本文中包含其他錯誤資訊，以便提供&#x200B;**有意義的上下文**&#x200B;來說明發生特定錯誤的原因及/或自動修正問題的可能解決方案。  *但是，在某些涉及驗證或登出流程的特定情況下，Adobe Pass驗證服務可能會傳回HTML回應或空白內文 — 如需詳細資訊，請檢視API檔案。*
...
處理多個專案的Adobe Pass API （預先授權API等）可能會使用專案層級錯誤資訊，指出處理特定專案是否失敗，以及其他專案是否成功。 在此情況下，***&quot;error&quot;***&#x200B;物件位於專案層級，而回應本文可能包含多個&#x200B;***&quot;errors&quot;***&#x200B;物件 — 請參閱API檔案。
...
| 名稱 | 型別 | 範例 | 受限制 | 說明 |
|---|---|----|:---:|---|
| *狀態* | *整數* | *403* | &amp;amp；檢查； | RFC 7231 (<https://tools.ietf.org/html/rfc7231#section-6>)中記錄的回應HTTP狀態碼 <ul><li>400錯誤請求</li><li>401未獲授權</li><li>403禁止</li><li>404找不到</li><li>405不允許的方法</li><li>409衝突</li><li>410已過期</li><li>412先決條件失敗</li><li>429太多請求</li><li>500間隔伺服器錯誤</li><li>503服務無法使用</li></ul> |
| *代碼* | *字串* | *network_connection_failure* | &amp;amp；檢查； | 標準Adobe Pass驗證錯誤代碼。 完整的錯誤代碼清單如下。 |
| *訊息* | *字串* | *無法連絡您的電視提供者服務* | | 可顯示給一般使用者的人類可讀訊息。 |
...
| *追蹤* | *字串* | *12f6fef9-d2e0-422b-a9d7-60d799abe353* | | 此回應的唯一識別碼，可在聯絡支援人員以識別更複雜案例中的特定問題時使用。 |
| *動作* | *字串* | *重試* | &amp;amp；檢查； | 補救此情況的建議動作： <ul><li> *無* — 很遺憾，沒有預先定義的動作來修正此問題。 這可能表示對公用API的呼叫不正確</li><li>*設定* — 需要透過TVE儀表板或連絡支援人員來變更設定。 </li><li>*應用程式註冊* — 應用程式必須重新註冊。 </li><li>*驗證* — 使用者必須驗證或重新驗證。 </li><li>*授權* — 使用者必須取得特定資源的授權。 </li><li>*退化* — 應該套用某種形式的退化。 </li><li>*重試* — 重試請求可能會解決問題</li><li>*在指定時段後重試* — 在指定時段後重試請求可能會解決問題。</li></ul> |

</br>

**附註：**

- ***受限制的***&#x200B;資料行&#x200B;*指出個別欄位值是否代表有限集合* （例如，&quot;*狀態*&quot;欄位的現有HTTP狀態碼）。 此規格的未來更新可能會將值新增至限制清單，但不會移除或變更現有值。 不受限制的欄位通常可以包含任何資料，但為確保合理的大小，可能有一些限制。

- 每個Adobe回應將包含「Adobe — 請求 — ID」，用於識別整個HTTP服務中的使用者端請求。 「**trace**」欄位補充該欄位，且這些欄位應一起報告。

## HTTP狀態碼和錯誤代碼 {#http-status-codes-and-error-codes}

由於與舊版sdk和應用程式(
範例*unknown\_application*&#x200B;產生400個錯誤請求，而&#x200B;*unknown\_software\_statement*&#x200B;產生401個未獲授權的請求。 解決這些不一致將在未來的反複專案中成為目標。

## 動作和錯誤代碼 {#actions-and-error-codes}

對於大多數的錯誤碼，多個動作可能符合資格作為修正手頭問題的路徑，或者甚至可能需要多個動作才能自動修正它們。 我們選擇指出修正錯誤的可能性最高的錯誤。 **動作**&#x200B;可以分割成三個類別：
...
對於第一個類別（重試和之後重試），僅重試相同請求可能足以解決問題。 如果API處理多個專案，應用程式應重複該請求，並僅包含具有「重試」或「在此之後重試」動作的專案。 對於「**」動作，「<u></u>」標頭將指出應用程式在重複要求之前應等待的秒數。

對於第2和第3個類別，實際動作實作高度取決於應用程式功能。 例如，「*降級*」可以實作為「切換至15分鐘暫時通路以允許使用者播放內容」，或實作為「自動工具，套用AUTHN-ALL或AUTHZ-ALL降級，以便與指定的MVPD整合」。 類似於「*驗證*」動作，可能會在平板電腦上觸發被動驗證（背景通道驗證），並在連線的電視上觸發完整的第二熒幕驗證流程。 這就是為什麼我們選擇提供包含結構描述和所有引數的完整URL。
...
|  | *個人化_服務_無法使用* | 503 | 由於無法使用個人化服務，請求失敗。 |
...
|  | *invalid_resource_value* | 400 | 要求失敗，因為使用的資源無效或格式錯誤。 請檢閱API檔案，以決定複雜資源必須如何針對您的請求編碼，以及其值和/或大小是否有任何限制。 |
|  | *invalid_registration_code* | 404 | 指定的註冊代碼不再有效或已過期。 |
|  | *無效的service_configuration* | 500 | 由於不正確的服務設定，請求失敗。 |
...
|  | *access_token_available* | 400 | 擷取存取Token時發生意外錯誤，導致要求失敗。 請檢查TVE儀表板設定，以取得可用的軟體陳述式及已註冊的自訂配置。 |
|  | *不支援的_client_version* | 400 | 此版本的Adobe Pass Authentication SDK太舊，不再受支援。 請檢視API檔案，瞭解升級至最新版本所需的步驟。 |
| **組態** | *network_required_ssl* | 403 | 目標合作夥伴服務發生SSL連線問題。 請聯絡支援團隊。 |
|  | *太多資源* | 403 | 授權或預先授權要求失敗，因為查詢的資源太多。 請聯絡支援團隊，以正確設定授權和預先授許可權制。 |
|  | *未知的程式設計員* | 400 | 無法辨識程式設計師或服務提供者。 使用TVE儀表板來註冊指定的程式設計人員。 |
|  | *未知的應用程式* | 400 | 無法辨識應用程式。 使用TVE儀表板來註冊指定的應用程式。 |
|  | *未知的整合* | 400 | 指定的程式設計師和身分提供者之間的整合不存在。 使用TVE Dashboard建立所需的整合。 |
|  | *unknown_software_statement* | 401 | 無法辨識與存取權杖關聯的軟體陳述式。 請連絡支援團隊，以釐清軟體宣告的狀態。 |
| **應用程式註冊** | *access_token_expired* | 401 | 存取權杖已過期。 應用程式應依照API檔案中的指示重新整理存取權杖。 |
|  | *invalid_access_token_signature* | 401 | 存取權杖簽章已無效。 應用程式應依照API檔案中的指示重新整理存取權杖。 |
|  | *無效的client_id* | 401 | 無法辨識關聯的使用者端識別碼。 應用程式應遵循API檔案中指示的應用程式註冊流程。 |
...
|  | *authentication_session_invalidated* | 401 | 身分提供者已使驗證工作階段失效。 使用者必須使用支援的MVPD重新驗證才能繼續。 |
...
| **在**&#x200B;之後重試 | *太多要求* | 429 | 在指定間隔內已傳送過多請求。 應用程式可在建議的時段後重試請求。 |
