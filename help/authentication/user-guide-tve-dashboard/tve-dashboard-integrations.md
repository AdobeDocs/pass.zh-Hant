...
>[!NOTE]
>
>MVPD可以為每個流程提供一個或多個端點。 整合新管道時，MVPD必須為每個流量指定其慣用的端點。
...
平台設定中的每個屬性都會繼承MVPD設定的預設值，但可視需要調整。

>[!IMPORTANT]
>
>您必須先與MVPD達成協定，才能決定為平台設定中的每個屬性設定的值。

>[!IMPORTANT]
>
> 設定繼承會遵循一個鏈結，從MVPD設定（最一般）開始，然後是MVPD端點、整合、平台類別和平台（包含最特定的值）。
...
>[!IMPORTANT]
>
>驗證(AuthN) TTL和授權(AuthZ) TTL值必須與MVPD設定一致。
...
對於不想建置其MVPD選擇器而是依賴提供的設定的程式設計師應用程式，您可以瀏覽至&#x200B;**新增更多屬性**，然後選取&#x200B;**顯示名稱**&#x200B;或&#x200B;**標誌URL**，以從選項功能表為每個MVPD新增必要的顯示名稱或標誌URL。
...
本節可讓您更新MVPD所共用的每個使用者中繼資料引數的設定。

>[!NOTE]
>
>每個MVPD可能會共用不同的引數。 如需特定MVPD可共用引數的詳細資訊，請洽詢您的Adobe代表。
...
1. 選取&#x200B;**整合**&#x200B;區段右上角的&#x200B;**建立新整合**。
   ...
   **選取端點**
   ...
   選取必要的MVPD後，**選取端點**&#x200B;區段將會預先填入為該特定MVPD設定的預設端點。
   ...
   **其他資訊**
   ...
   此區段包含需要為&#x200B;**選取頻道和MVPD**&#x200B;區段中所選MVPD設定的各種屬性。

   >[!NOTE]
   >
   > 根據&#x200B;**選取頻道和MVPD**&#x200B;區段中選取的MVPD，實際屬性可能會有所不同。
