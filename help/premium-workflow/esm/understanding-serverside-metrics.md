...
* **在驗證流程中產生的事件** (實際使用MVPD登入)

   * 驗證嘗試通知 — 這會在使用者傳送至MVPD登入網站時產生。
   * 驗證通知擱置中 — 如果使用者成功使用其MVPD登入，這會在使用者執行以下動作時產生        已重新導向回Adobe Pass驗證。
...
在這種情況下，重複資料刪除應該不會有任何影響，因為不同的MVPD使用者不應該收到相同的使用者ID。 針對同一個MVPD中的兩個不同品牌進行加總時，重複資料刪除的效果應該會更明顯。
...
如上圖所示，只有當使用者往返至MVPD時，才會觸發驗證事件；驗證事件不包含快取權杖驗證。

使用者從選擇器按一下特定的MVPD後，就會觸發驗證嘗試事件。

* MVPD端與此事件相鄰的第一個事件是頁面載入
...
使用者是MVPD的已知訂閱者，通常是付費電視訂閱，但有時僅能存取網際網路。 發生成功的驗證可能是因為使用者透過其MVPD明確輸入有效認證，或是因為他們先前輸入有效認證並勾選「記住我」（且先前的工作階段未過期）。
...
請注意，並非所有MVPD都支援個別的授權步驟；對於某些驗證，授權是等同的。 MVPD將Adobe Pass驗證成功回應傳送至後端通道AuthZ請求，而Adobe Pass驗證會建立AuthZ權杖。
...
對於某些MVPD，即使使用者只有MVPD的網際網路訂閱（無付費電視訂閱），仍可成功驗證。 在這種情況下，即使使用者請求授權的管道位於基本套件中，授權也會被拒絕。
...
* 由於AuthN和AuthZ權杖會快取一段時間的MVPD回應，因此成功的播放請求事件是量度中最常見的事件。
...
此值可以跨裝置持續存在(MVPD會為指定使用者產生相同的值，無論登入發生在何處)或暫時性(每次登入都會產生新值，而MVPD會在後端對應此值)。 通常MVPD提供給Adobe Pass驗證的值會在工作階段和裝置之間持續存在，但如前所述，持續性既不保證也不驗證。

此值用於計算不重複使用者。 所報告的值(根據請求者ID/間隔/MVPD)會在特定間隔內刪除重複專案。 因此，每天不重複使用者的總和通常與每月值不同，每月值的值較低。
