...
CMU API提供基礎OLAP立方結構的階層檢視。 每個資源（[維度](/help/concurrency-monitoring/reports/cm-usage-reports.md#dimensions-2-filter-metrics)，在維度階層中，對應為URL路徑區段）會產生包含（彙總） [個量度](/help/concurrency-monitoring/reports/cm-usage-reports.md#monitor-metrics)的報表，以用於目前的選取專案。 每個資源都指向其父資源（用於累計）及其子資源（用於向下鑽研）。 切片和切割是透過將維度釘選到特定值或範圍的查詢字串引數實現的。
...
`GET` API端點的`https://mgmt.auth.adobe.com/cmu/v2`將傳回包含：
...
目前唯一可用的HTTP方法是GET。 未來版本可能會提供對OPTIONS / HEAD方法的支援。
...
* [CMU概述](/help/concurrency-monitoring/reports/cm-usage-reports.md)
