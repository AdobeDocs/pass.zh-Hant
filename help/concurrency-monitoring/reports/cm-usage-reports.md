...
**並行監視使用量報告**&#x200B;服務可透過REST API使用，API可將insight提供給客戶應用程式報告的並行使用量。

## 先決條件 {#usage-rep-prerequisites}

若要存取「並行監視使用報告」產品，客戶必須首先連絡並行監視[支援團隊](mailto:tve-support@adobe.com)，他們將執行必要的步驟以允許您存取API產品。 有關[CMU API存取](/help/concurrency-monitoring/reports/cmu-api-access.md)的更多詳細資料。
...
為達到最佳資料準確度，建議您在此頁面上呈現的報告[CMU報告範例](/help/concurrency-monitoring/reports/cm-usage-reports-examples.md)
