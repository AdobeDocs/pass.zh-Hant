...
>[!NOTE]
>
>請致電熱線： +1-657-312-4623，並傳送電子郵件至`tve-support@adobe.com`，主旨行中包含「緊急 — 事件」。
...
* 在驗證程式期間，使用者卡在Adobe錯誤頁面上，無法重新啟動驗證/授權流程。
...
* 由於Flash版本問題（缺少Flash、Flash封鎖程式、錯誤的Flash版本），有一或數個訂閱者無法執行流程。
...
1. 在影響分析期間，如果Adobe確定問題屬於其中一個預先協定的事件情境決策（請參閱以下「預先協定的事件情境決策」小節），它將做出相應的行動，而不等待合作夥伴1。 的輸入。

1. Adobe將等待合作夥伴的更新，以及服務還原時合作夥伴的通知。
...
|    | 情境 | 說明 | 動作 |
|:--:|:------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| S1 | Adobe會在正常生產作業期間識別MVPD整合的問題。 | 在正常生產作業期間，Adobe會識別其中一個MVPD的問題，此問題會導致無法執行驗證/授權流程（例如過期的憑證、過期的SAML回應、關閉的連線埠、變更的引數等） | Adobe將通知受影響的MVPD和程式設計師。 Adobe將會為所有受影響的程式設計師停用此MVPD。 Adobe將依照與該MVPD協定的支援程式，透過該MVPD開啟票證 |
| S2 | Adobe為程式設計師啟用新的MVPD，而程式設計師在啟動日期前將MVPD加入白名單。 | Adobe正在為程式設計師的網站啟用新的MVPD，而且該網站已在選擇器中顯示新的MVPD，即使它不應該顯示。 | Adobe會在排程日期之前，將新MVPD出現在選擇器中的情況通知程式設計師。 如有必要，程式設計師會採取動作將它從選擇器中移除。 |
| S3 | Adobe為程式設計師啟用新的MVPD，即使MVPD尚未準備好投入生產 | Adobe正在為程式設計師啟用新的MVPD，但MVPD尚未部署整合的支援，因此無法執行驗證/授權流程 | Adobe只有在程式設計師提出要求時才會進行部署。程式設計師將負責確保所有測試執行完畢後，才會將MVPD加入白名單。 |
